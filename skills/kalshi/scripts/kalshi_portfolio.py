...
DEFAULT_CREDENTIALS_PATH = Path.home() / ".kalshi" / "credentials.json"
...
def load_credentials() -> dict:
    ...
    credentials_path = Path(
        os.environ.get("KALSHI_CREDENTIALS_PATH", str(DEFAULT_CREDENTIALS_PATH))
    ).expanduser()
    if not credentials_path.exists():
        return None
    with open(credentials_path) as f:
        creds = json.load(f)
    creds["source"] = str(credentials_path)
    return creds
...
def get_client() -> KalshiClient:
    ...
    if not HAS_SDK:
        print("Error: kalshi-python not installed or vendored for this skill.")
        sys.exit(1)
    
    creds = load_credentials()
    if not creds:
        print("Error: No Kalshi credentials found.")
        print("Set KALSHI_API_KEY_ID + KALSHI_PRIVATE_KEY_PATH,")
        print("or KALSHI_API_KEY_ID + KALSHI_PRIVATE_KEY_PEM,")
        print(f"or create {DEFAULT_CREDENTIALS_PATH}")
    ...
    private_key = creds.get("private_key_pem")
    ...
    if not api_key_id:
        print("Error: Kalshi API key ID is missing.")
        sys.exit(1)
