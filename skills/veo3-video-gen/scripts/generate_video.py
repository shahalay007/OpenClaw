...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of retries for transient API failures per segment (default: 2)",
    )
    ...
    for i in range(1, args.segments + 1):
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                print(
                    f"Starting video generation segment {i}/{args.segments} "
                    f"(model={args.model}, attempt={attempt}/{max_attempts})…"
                )
                ...
                print(
                    f"Transient Veo failure for segment {i} (attempt {attempt}/{max_attempts}): {e}",
                    file=sys.stderr,
                )
                print(f"Retrying in {backoff_seconds}s…", file=sys.stderr)
                time.sleep(backoff_seconds)

        if last_error is not None:
            print(f"Error generating segment {i}: {last_error}", file=sys.stderr)
            sys.exit(2)
