...
This repo vendors the required Python SDK inside the skill, so prefer `python3` directly. `uv` is optional and not required in the OpenClaw container.
...
- Vendored `google-genai` ships with this repo skill; no separate install is required.
